...
// interface GalleryGridPropsType {
//     gallery: {
//         img: string;
//         id: number;
//     }[];
...
// export default function GalleryGrid({ gallery } : GalleryGridPropsType) {
...
export default function GalleryGrid({ gallery } : GalleryGridPropsType) {
    ...
    const posts: GalleryImageType[] = imageData;
    const postsPerPage = 9;
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [pageItem, setPageItem] = useState<{ start: number; end: number }>({
        start: 0,
        end: postsPerPage,
    });
    ...
    useEffect(() => {
        let tempNumberOfButtons: (number | string)[] = [];
        const dotsInitial = '...';
        const dotsLeft = '... ';
        const dotsRight = ' ...';
                            ...
                            {arrOfCurrButtons.map((data, index) => {
                                ...
                                return (
                                    <span key={index} className={`dt-item ${currentPage === data ? 'active' : ''} border-none`}>
                                        <button
                                            className={`dt-link px-[16.5px] py-[10px] border-none font-inter text-[14px] font-medium leading-[20px] rounded-[8px] transition-all ${currentPage === data ? 'bg-[#F9FAFB] text-[#182230]' : 'bg-white'}`}
                                            onClick={() => setCurrentPage(data as number)}
                                        >
                                            {data}
                                        </button>
                                    </span>
